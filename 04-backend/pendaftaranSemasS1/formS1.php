...
function selectAllProdiS1() {
  ...
  $query = "select nama,jenis_kelas,kode from tk_basdat_a02.PROGRAM_STUDI where jenjang = 'S1' and  exists (select nomor_periode,tahun_periode,kode_prodi from tk_basdat_a02.PENERIMAAN_PRODI where exists (select tahun from tk_basdat_a02.periode_penerimaan where status_aktif = true))";
